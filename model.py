...
class Encoder(nn.Module):
    def __init__(self, n_vocab, n_embed, n_hidden, n_layer, vocab=None):
        ...
        if vocab is None:
            ...
            self.embedding = nn.Embedding(self.n_vocab, self.n_embed)
            vectors = KeyedVectors.load_word2vec_format(datapath('./sgns.weibo.bigram-char'), binary=False)
            int i = 0;
            for word in vectors.wv.vocab.item():
                embedding[i] = torch.Tensor(vectors[word]);
            self.embedding = nn.Embedding.from_pretrained(embedding)
            print("encoder embedding is initialized with Chinese")    
            self.gru = nn.GRU(input_size=self.n_embed, hidden_size=n_hidden,
...
class KnowledgeEncoder(nn.Module):
    def __init__(self, n_vocab, n_embed, n_hidden, n_layer, vocab=None):
        ...
        #             embedding[vocab.stoi[word]] = vectors[word]
        #     self.embedding = nn.Embedding.from_pretrained(embedding)
        #     print("Kencoder embedding is initialized with Chinese")
        # self.gru = nn.GRU(input_size=n_embed, hidden_size=n_hidden,
        #                   num_layers=n_layer, bidirectional=True)
